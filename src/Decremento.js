...
var DECREMENTO = /** @class */ (function (_super) {
    ...
    DECREMENTO.prototype.DECREMENTO = function (val) {
        this.Nombre = val;
        this.id = 0;
        this.linea = 0;
        this.columna = 0;
    ...
    DECREMENTO.prototype.Ejecutar = function (entorno) {
        ...
        for (var i = 0; i < entorno.VariableVariablesFUNCION.length; i++) {
            if (Variable.toUpperCase() == entorno.VariableVariablesFUNCION[i].toUpperCase()) {
                if (entorno.EsConstante[i] == 1) {
                    ...
                    Bandera = "true";
                    if (signo == "+") {
                        entorno.direccion += "##Vamos A Incrementar\n";
                        var tes = entorno.TesVariablesFUNCION[i];
                        entorno.numero += 1;
                        entorno.direccion += "t" + entorno.numero + " = stack[" + tes + "];\n";
                        entorno.numero += 1;
                        entorno.direccion += "t" + entorno.numero + " = t" + (entorno.numero - 1) + " + 1;\n";
                        entorno.direccion += "stack[" + tes + "] = t" + entorno.numero + ";\n";
                    }
                    else {
                        entorno.direccion += "##Vamos A decrementar\n";
                        var tes = entorno.TesVariablesFUNCION[i];
                        entorno.numero += 1;
                        entorno.direccion += "t" + entorno.numero + " = stack[" + tes + "];\n";
                        entorno.numero += 1;
